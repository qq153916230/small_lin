...
<script type="text/javascript">
	...
	function bindDatagrid() {
		...
		datagrid = $('#dg').datagrid({
							...
							url : '<%=path %>/epayTrade/selectList?regTimeFrom='+regTimeFrom
					...
					{ field: 'master', title: '信用卡卡主', width: 100,  align: 'center', sortable: 'true' },
					...
					{ field: 'amount', title: '金额', width: 100, formatter: formatprice, align: 'center', sortable: 'true' },
					...
					{ field: 'gdate', title: '创建日期', width: 200,  align: 'center', sortable: 'true' ,
					formatter : function(value) {
				            var date = new Date(value);
				            var year = date.getFullYear().toString();
				            var month = (date.getMonth() + 1);
	...
	//判断达标时间
       ...
       function countMoney(){
       	$.ajax({
       		url : '<%=path %>/epayTrade/selectCountMoney?regTimeFrom='+regTimeFrom
					+'&regTimeTo='+regTimeTo+'&mobile='+mobile+'&searchStatus='+searchStatus,
			type:'post',
			dataType:'text',
	...
	<form  id="form1">
		...
		<div style="display:none;">
			<div id="toolbar">
				...
				<span>收款总额：</span><span id="countMoney" style="color:red;">0.00</span>元
			</div>
		</div>
	</form>
