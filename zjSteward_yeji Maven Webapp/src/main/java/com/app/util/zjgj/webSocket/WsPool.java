...
public class WsPool {
    ...
    public static void addUser(String userName, WebSocket conn) {
        ...
        System.out.println("save user to wsPool："+userName +" poolSize:" + wsUserMap.size());
    }
    ...
    public static boolean removeUser(WebSocket conn) {
    	System.out.println("user leaved:"+conn);
        if (wsUserMap.containsKey(conn)) {
        	String uname = wsUserMap.get(conn);
            wsUserMap.remove(conn); // 移除连接
            System.out.println("user leaved：" + uname + " poolSize:"+wsUserMap.size());
            return true;
        } else {
            return false;
        }
