...
public class WsServer extends WebSocketServer {
    ...
    public void onOpen(WebSocket conn, ClientHandshake handshake) {
    	...
    	System.out.println("用户连接到socket："+conn + " "+ ZjUtils.getDateStr("yyyy-MM-dd HH:mm:ss"));
    ...
    public void onClose(WebSocket conn, int code, String reason, boolean remote) {
        //断开连接时候触发代码
        userLeave(conn);
        System.out.println(conn + " " + reason + " "+ ZjUtils.getDateStr("yyyy-MM-dd HH:mm:ss"));
    }
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        if(null != message &&message.startsWith("online")){
            String userName=message.replaceFirst("online", "").trim();//用户名
